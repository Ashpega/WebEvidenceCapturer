...
if ($flag -eq 0) {
    wsl bash -c "source ~/dev/myenv_withPlayWright/bin/activate && python3 ~/dev/myenv_withPlayWright/OpenChromium.py '$wslScriptUrlTxtPath' '$output_dir_name' '$base_name'"
...
if ($newFile) {
   ...
   wsl bash -c "source ~/dev/myenv_withPlayWright/bin/activate && python3 ~/dev/myenv_withPlayWright/CloseChromium.py '$wslScriptFullHtmlPath' '$output_dir_name' '$base_name' '$wslScriptUrlTxtPath'" 


   if ($LASTEXITCODE -ne 0) {
      Write-Host "An error occurred while CloseChromium.py (CODE: $LASTEXITCODE)"
      }
}
