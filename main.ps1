...
$url_txt_path = "$PSScriptRoot/tmp_url.txt"
...
if ($flag -eq 0) {
    wsl bash -c "source ~/myenv_withPlayWright/bin/activate && python3 ~/myenv_withPlayWright/ObtainPngHtml.py '$wslScriptUrlTxtPath' '$output_dir_name' '$base_name'"
...
Start-Process "SingleFileSave1.ahk"
...
Start-Process "CloseChromeTab1.ahk"
...
if ($newFile) {
   ...
   wsl bash -c "source ~/myenv_withPlayWright/bin/activate && python3 ~/myenv_withPlayWright/GetHashOtsofFullHtml.py '$wslScriptFullHtmlPath' '$output_dir_name' '$base_name' '$wslScriptUrlTxtPath'" 


   if ($LASTEXITCODE -ne 0) {
      Write-Host "An error occurred while GetHashoffullHTML (CODE: $LASTEXITCODE)"
      }
}
